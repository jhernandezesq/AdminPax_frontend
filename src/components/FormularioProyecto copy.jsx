...
const FormularioProyecto = ({ tarea }) => {
  ...
  const guardar = (e) => {
    const firma_cliente = sigCanvas.current
      .getTrimmedCanvas()
      .toDataURL("image/png");
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      [
        orden,
        nombre /* , direccion, colonia, ciudad, tel, cp, estado, solicitud_fecha, hora, visita_fecha, hora_entrada, hora_salida, reporte_cliente, acciones_realizadas, seguimiento, material_utilizado, comentarios_cliente, firma_pax, firma_cliente */,
      ].includes("")
    ) {
      mostrarAlerta({
  ...
  return (
    <div className="max-w-8xl mx-auto bg-white px-3 py-2">
      <form className="grid  mb-6 grid-cols-1 " onSubmit={handleSubmit}>
        ...
        <div>
          {firma_pax === "" || firma_cliente ? (
            <p>Firma deshabilitada</p>
          ) : (
            <SignaturePad
              id="fileInput"
              value={firma_cliente}
        ...
        <input
          ...
          className="text-center bg-sky-600 w-full p-3 font-bold text-white rounded cursor-pointer hover:bg-sky-700 transition-colors"
        ></input>
      </form>
    ...
    <img src={firma_cliente} alt="" />
      ...
      <p className="mt-5 text-white">
        ...
        <a className="text-white hover:underline" href="/" target="_blank">
